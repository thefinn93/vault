...
- `vault-active` `(string: "true"/"false")` – Vault active is updated dynamically each time Vault's active status changes.
  **This field is only valuable if the pod is NOT a member of a performance standby cluster**,
  it will simply be set to "false" when it's not applicable. True indicates that this Vault pod is currently the leader in a cluster.
  False indicates that this Vault pod is currently a standby _if not in a performance cluster_.
...
- `vault-perf-standby` `(string: "true"/"false")` – Vault performance standby is updated dynamically each
  ...
  that this Vault pod is currently a performance leader _if in a performance cluster_.
...
effectively allowing direct communication with subsets of Vault pods:
...
By setting `publishNotReadyAddresses: false`, pods that have failed will be removed from the service pool.  Upon
rescheduling of the pod, the label will be automatically updated to reflect its new role.
